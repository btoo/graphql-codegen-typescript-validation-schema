...
class ZodSchemaVisitor extends schema_visitor_js_1.BaseSchemaVisitor {
    ...
    typenameOptionality = this.config.nonOptionalTypename ? '' : '.optional()';
                ...
                switch (this.config.validationSchemaExportType) {
                    case 'const':
                        return (new visitor_plugin_common_1.DeclarationBlock({})
                            ...
                            .withContent([
                            `z.object({`,
                            (0, visitor_plugin_common_1.indent)(`__typename: z.literal('${node.name.value}')${this.typenameOptionality},`, 2),
                            shape,
                            '})',
                        ].join('\n'))
                            .string + appendArguments);
        ...
        if (field.kind === graphql_1.Kind.INPUT_VALUE_DEFINITION) {
            ...
            if (defaultValue?.kind === graphql_1.Kind.STRING || defaultValue?.kind === graphql_1.Kind.ENUM) {
                if (config.useEnumTypeAsDefaultValue && defaultValue?.kind !== graphql_1.Kind.STRING) {
                    let value = (0, visitor_plugin_common_1.convertNameParts)(defaultValue.value, (0, plugin_helpers_1.resolveExternalModuleAndFn)('change-case-all#pascalCase'), config.namingConvention?.transformUnderscore);
                    if (config.namingConvention?.enumValues)
                        value = (0, visitor_plugin_common_1.convertNameParts)(defaultValue.value, (0, plugin_helpers_1.resolveExternalModuleAndFn)(config.namingConvention?.enumValues), config.namingConvention?.transformUnderscore);
